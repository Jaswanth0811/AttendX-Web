...
export default function FacultyLayout({ children }: { children: React.ReactNode }) {
  const auth = useRequireAuth('faculty');
  ...
  if (auth.isLoading || !auth.isAuthenticated || auth.user?.role !== 'faculty') {
    return (
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 text-gray-700 dark:text-gray-300">
      ...
      <div className="h-16 flex items-center gap-2.5 px-6 border-b border-gray-100 dark:border-gray-800">
        <div className="bg-gradient-to-tr from-purple-500 to-indigo-600 p-2 rounded-lg text-white">
          <Users className="w-5 h-5" />
        </div>
        {!isCollapsed && (
          <span className="font-bold text-lg bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent dark:from-purple-400 dark:to-indigo-400">
            AttendX Faculty
          </span>
        )}
      </div>
      ...
      <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
        {sidebarItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400 dark:text-gray-500'}`} />
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-800 space-y-2">
        {!isCollapsed && (
          <div className="flex items-center gap-3 px-3 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-800/40">
            <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-purple-400 to-indigo-500 flex items-center justify-center text-white font-bold text-sm">
              DR
            </div>
        ...
        <header className="h-16 flex items-center justify-between px-6 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-purple-100 dark:bg-purple-950/60 flex items-center justify-center text-purple-600 dark:text-purple-400 font-bold text-sm">
                DR
              </div>
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300 hidden sm:block">Faculty Portal</span>
            </div>
          </div>
        </header>
