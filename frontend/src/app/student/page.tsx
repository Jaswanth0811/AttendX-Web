...
import { QrCode, AlertTriangle, CheckCircle2, XCircle, TrendingUp, Info } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-gradient-to-r from-teal-500/10 to-emerald-500/10 border border-teal-500/20 dark:from-teal-950/20 dark:to-emerald-950/20">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Hey Rahul Sharma,</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border-0 shadow-sm dark:bg-gray-900/60 p-6 flex flex-col items-center justify-center text-center">
          ...
          <CardContent className="space-y-4 flex flex-col items-center">
            ...
            <div className="relative w-36 h-36 flex items-center justify-center rounded-full border-8 border-teal-500/10 dark:border-teal-500/5 mt-2">
              <div className="text-center">
                <span className="text-4xl font-extrabold text-teal-600 dark:text-teal-400">{dashboard.overallAttendance}%</span>
                <span className="text-[10px] text-gray-400 block font-bold uppercase mt-0.5">Semester 3</span>
              </div>
            </div>

            {/* Threshold Status */}
            <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20 dark:text-emerald-400 px-3 py-1 rounded-full">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {dashboard.subjectWise.map((sub, idx) => (
              <Card key={idx} className="border-0 shadow-sm dark:bg-gray-900/60 overflow-hidden">
                <CardHeader className="pb-2">
        ...
        <Card className="lg:col-span-2 border-0 shadow-sm dark:bg-gray-900/60">
          ...
          <CardContent className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={dashboard.trend} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
