...
import { Hash, ShieldCheck, RefreshCw, AlertCircle, ScanLine } from 'lucide-react';
...
export default function StudentQRScannerPage() {
  ...
  const handleMarkAttendance = async (code: string) => {
    ...
    try {
      const res = await apiFetch('/api/student/attendance/mark', {
        method: 'POST',
      ...
      if (res.success && res.data) {
        ...
        setScannedDetails({
          subject: `${session.subjectCode} - ${session.subjectName}`,
          section: session.sectionName,
          time: new Date(res.data.markedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          faculty: session.facultyName,
        });
        setScanStatus('success');
      } else {
        setErrorMessage(res.message || 'Invalid or expired code. Please check with your lecturer.');
        setScanStatus('error');
      }
    } catch (err: any) {
            ...
            <form onSubmit={handleManualSubmit} className="space-y-6">
              <div className="flex flex-col items-center space-y-4">
                ...
                <div className="text-center space-y-1">
                  <h3 className="font-bold text-lg">Enter Attendance Code</h3>
                  <p className="text-xs text-muted-foreground">The 6-digit alphanumeric code rotates based on active session rules.</p>
                </div>
              </div>
        ...
        {scanStatus === 'error' && (
          <CardContent className="pt-8 pb-8 text-center space-y-4 flex flex-col items-center">
            ...
            <div className="space-y-1 max-w-sm">
              <h3 className="font-bold text-lg text-red-655 dark:text-red-400">Submission Failed</h3>
